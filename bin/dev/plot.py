...
def main():
    ...
    trans_sn = {'braker1' : [47.83, 47.83, 47.83, 48.57, 48.57, 48.57, 27.92, 44.33, 44.33, 44.33, 40.89],
            ...
            'test1' :       [54.16, 52.05, 51.19, 51.08, 46.75, 46.54, 31.49, 49.98, 45.61, 44.6, 47.52],
            'test2' :       [54.41, 52.45, 51.68, 50.96, 46.59, 46.41, 31.97, 50.25, 46.12, 44.88, 47.77]}
    ...
    trans_sp = {'braker1' : [57.94, 57.94, 57.94, 58.44, 58.44, 58.44, 24.08, 57.78, 57.78, 57.78, 37.46],
            ...
            'test1' :       [71.99, 71.34, 70.03, 71.1, 67.68, 67.7, 24.72, 72.51, 68.16, 66.9, 44.23],
            'test2' :       [75.76, 75.58, 74.48, 72.91, 69.49, 69.51, 31.85, 76.85, 72.17, 70.37, 50.66]}
    ...
    tests = {'braker1' : 'tab:orange', 'braker2' : 'tab:red', 'test1' : 'tab:green', 'test2' : 'tab:blue'}
    x = np.arange(len(label))
    fig, ((ax1, ax2)) = plt.subplots(2, 1)
    fig.subplots_adjust(hspace=.05)
    fig.suptitle('CDS Accuracy', fontsize=40)
    wid = 0.14
    i = -1.5 * wid
    ...
    for t in tests.keys():
        ax1.bar(x +i, cds_sn[t], color=tests[t], width=wid)
        i += wid
    i = -1.5 * wid
    ax2.set_ylabel('Specificity', fontsize=30)
    ax2.set_ylim(0,100)
    for t in tests.keys():
        ax2.bar(x +i, cds_sp[t], color=tests[t], width=wid)
        i += wid
    fig.legend(["BRAKER1", "BRAKER2", "Decision_Rule", "Decision_Rule+"], prop={'size': 30})
    '''
    i = -1.5 * wid
    for t in tests.keys():
