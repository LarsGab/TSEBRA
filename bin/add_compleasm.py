...
Description: This script runs compleasm on a genome with a given BUSCO partition
             and parses the output table for complete BUSCOs to generate a gtf file
             with genes that is compatible with BRAKER and GALBA.
...
# install instructions for compleasm that will be needed for containers:
# 
...
def main():
    ...
    #run_simple_process([args.compleasm, 'run', '-l', args.database, '-a', args.genome, '-t', args.threads, '-o', 'compleasm_genome_out'])

    # parse compleasm output table for complete BUSCOs without frameshifts
    busco_ids = extract_tx_ids_from_tsv('compleasm_genome_out/' + args.database + '/full_table.tsv')
    
    gff_lines = read_and_filter_gff('compleasm_genome_out/' + args.database + '/miniprot_output.gff', busco_ids)
